...
```
/contentwriting → /refine → /publish → /framer (optional)
```

`/publish` runs after `/refine` produces the -final.md with YAML frontmatter. It reads but does not modify the article file.
...
<!-- Generated by /publish — paste into CMS <head> or Custom Code section -->
