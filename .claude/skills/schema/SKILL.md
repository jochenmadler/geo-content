...
If the article has no YAML frontmatter, tell the user in your response (not in the output file): "This article has no YAML frontmatter. Run `/refine` on a draft created with contentwriting v3.4.0+ to get frontmatter. You can also add it manually."
...
If Section 8 is missing, tell the user in your response (not in the output file): "Company context is missing Section 8 (Structured Data & Publishing). Run `/company-context` to update it. Generating schemas with available data."
...
If any check fails, fix it before output. If a required URL is missing (e.g., no blog base URL), add a customer-friendly HTML comment like `<!-- Blog base URL not configured — update before publishing -->`. Never reference internal skill names, commands, or section numbers in the output file.
...
<!-- Paste into CMS <head> or Custom Code section -->
...
- **Customer-facing output**: The .html file is handed to the customer. Never include internal skill names (`/schema`, `/refine`, `/company-context`), internal document references ("Section 8"), or diagnostic notes about missing data sources. Warnings about missing inputs go in your response message to the user, not in the output file. HTML comments in the file should only contain information useful to the person pasting the code (e.g., "Paste into CMS `<head>`", "Blog base URL not configured").
